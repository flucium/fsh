...
mod tests {
    ...
    const TEST_FD_3: RawFd = 3;
    const TEST_FD_4: RawFd = 4;
    ...
    fn test_pipe_open() {
        ...
        assert_eq!(pipe.state(), &PipeState::Sendable);

        assert!(pipe.fd().is_none());
    }

    #[test]
    fn test_pipe_send_and_recv() {
        let mut pipe = Pipe::open();

        assert!(pipe.send(TEST_FD_3).is_ok());

        assert_eq!(pipe.state(), &PipeState::Recvable);

        assert_eq!(pipe.recv().unwrap(), TEST_FD_3);

        assert_eq!(pipe.state(), &PipeState::Sendable);
    }

    #[test]
    fn test_pipe_send_fail_if_already_set() {
        let mut pipe = Pipe::open();

        pipe.send(TEST_FD_3).unwrap();

        assert!(pipe.send(TEST_FD_4).is_err());
    }

    #[test]
    fn test_pipe_recv_fail_if_empty() {
        let mut pipe = Pipe::open();

        assert!(pipe.recv().is_err());
    }
