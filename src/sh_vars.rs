...
impl ShVars {
    ...
    /// Returns the number of shell variables in the collection.
    ///
    /// This function returns the total count of key-value pairs stored in the `ShVars` instance.
    ///
    /// # Returns
    /// - `usize`: The number of shell variables.
...
mod tests {
    ...
    fn test_shvars_keys_and_values() {
        ...
        for i in 0..100 {
            shvars.insert(format!("key{i}"), format!("value{i}"));
        }
