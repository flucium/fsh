...
/// PipeState is an enum that represents the state of the Pipe.
...
impl PipeState {
    /// Convert PipeState to u32.
    pub fn as_u32(self) -> u32 {
        self as u32
    }

    /// Convert PipeState to usize.
    pub fn as_usize(self) -> usize {
        self as usize
    }

    /// Convert u32 to PipeState.
    pub fn from_u32(value: u32) -> Result<Self> {
        match value {
            0 => Ok(PipeState::Closed),
            1 => Ok(PipeState::Sendable),
            2 => Ok(PipeState::Recvable),
            _ => Err(Error::new(ErrorKind::InvalidInput, "invalid pipe state")),
        }
    }

    /// Convert usize to PipeState.
